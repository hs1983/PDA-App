...
export default {
  ...
  methods: {
    ...
    submitPutin() {//提交入库
      ...
      //多个订单组一托盘，所有订单包件需要全部扫完
      // const objOrder = {};//用于标记字符串
      // const arrOrderList = [];//去掉重复后的订单
      // for (var i = 0, len = this.fItems.length; i < len; i++) {
      //   const s = this.fItems[i].orderNo;
      //   if (objOrder[s]) continue;//如果字符串已经存在就跳过
      //   else {
      //     objOrder[s] = s;//加入标记对象中
      //     arrOrderList.push(s);//结果放入新数组中
      //   }
      // }

      // if(arrOrderList.length>1){//是否是多订单组一托盘
      //   const f=this.fItems.length;
      //   const a=this.aItems.length;
      //   if(f<a){//包件是否全部扫描完
      //     this.$toast("请先扫描订单所有包件，再确认组盘!");
      //     return;
      //   }
      // }
    ...
    getNoScanning() {//没有扫描的包件
      this.nItems = [];
      this.aItems = [];
      let srcList = this.dataList;//所有包件
      srcList.forEach((itm, index, srcList) => {
        this.fItems.forEach((ele) => {
          if (itm.packCode === ele.packCode) {
      ...
      //全部包件
      let tmpItem = this.fItems;

      for (let i in tmpItem) {
        tmpItem[i].scanned = true;
        this.aItems.push(tmpItem[i])
      }

      for (let i in srcList) {
        this.aItems.push(srcList[i])
      }
    ...
    checkPackCode(barcode) {//校验 包件条码
      if (barcode.length > 0) {
        ...
        if (arrCode.length > 1) {
          ...
          // if (!(barcode.toUpperCase().indexOf('WJ') === 0)) {//条码不是以WJ开头
          //   let arrBarCode = barcode.split(",");
          //   if (arrBarCode.length > 1) {
          //     arrBarCode.forEach(element => {
          //       this.barCodeQuery(element);
