<template>
  <div id="completeGrouping">
    <mt-header style="background-color: #2449a4" title="拣货装车">
...
import { getPalletizeList, submitPackInfo } from "@/apis/package"

export default {
  data() {
    return {
      isQuickScan: false,//是否支持快捷扫描(一个托盘只扫一个包件)
      isAShow: false,
      isFShow: false,
      isNShow: false,
  ...
  methods: {
    ...
    submitPutin() {//提交入库
      ...
      Indicator.open();
      submitPackInfo(this.fItems).then((response) => {
    ...
    getPalletComb(barcode) {//调用接口获取订单包件信息
      ...
      return new Promise((resolve, reject) => {
        getPalletizeList(barcode).then((response) => {
          if (response.code === 1) {
